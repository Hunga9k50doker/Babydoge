...
class Babydoge {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (attempts < maxAttempts) {
          ...
          this.log(
            `Error khi kiểm tra IP của proxy sau ${maxAttempts} lần thử: ${error.message}`
          );
          break;
        }
      }
  ...
  async dangnhap(tgData, proxy) {
    ...
    try {
      ...
      if (res.data) {
        const points_per_tap = res.data.league.points_per_tap;
        return { points_per_tap, ...res.data };
      } else {
  ...
  async claimTask(access_token, channel, proxy) {
    ...
    try {
      ...
      if (res && res.data) {
        this.log(
          `Đang làm nhiệm vụ: ${channel.title.yellow}... Trạng thái: thành công`
        );
      } else {
  ...
  async tapdc(access_token, initialEnergy, points_per_tap, proxy) {
    ...
    try {
      while (energy >= 50) {
        ...
        if (count <= 0) {
          this.log(
            "Năng lượng không đủ để tiếp tục tap...chuyển tài khoản!".yellow
          );
          break;
        }
        ...
        if (res.data) {
          ...
          this.log(
            `Đã tap ${String(mined).yellow} lần. Balance: ${
              String(balance).yellow
            } Năng lượng: ${String(newEnergy).yellow}`
          );
          ...
          if (energy < 50) {
            this.log(
              "Năng lượng quá thấp để tiếp tục tap...chuyển tài khoản!".yellow
            );
            break;
          }
        } else {
  ...
  async main() {
    ...
    if (upgradeMyCardsDecision) {
      ...
      if (isNaN(maxUpgradeCost) || maxUpgradeCost <= 0) {
        this.log(
          "Giá trị không hợp lệ. Sẽ sử dụng giá trị mặc định là không giới hạn."
            .yellow
        );
    ...
    while (true) {
      ...
      for (const [index, tgData] of data.entries()) {
        ...
        const { balance, access_token, energy, max_energy, points_per_tap } =
          loginData;
